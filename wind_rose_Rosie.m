...
% percBg='w';
percBg='none';
% percBg=[250/255,250/255,250/255]; % very very light grey
titStr='';
legStr='';
cmap=jet;
colors=[];
...
for i=1:ncircles
  ...
  labs(i)=text((ci(i)+ri)*cos(Ang(quad)),(ci(i)+ri)*sin(Ang(quad)),[num2str(ci(i)),'%'],...
      ...
      'BackgroundColor',percBg,'fontsize',12);
end
...
% N S E W labels:
bg='none';
args={'BackgroundColor',bg,'fontsize',10};
...
if legType==1 % continuous
  for j=1:length(Ag)-1
    ...
    handles(end+1)=text(x1+L/4,y0,lab,'VerticalAlignment','middle','fontsize',12);
    y0=y1;
  end
  if ~ (hasH & IncHiLow)
    handles(end+1)=text(x1+L/4,y0,num2str(Ag(end)),'VerticalAlignment','middle','fontsize',12);
  end
elseif legType==2 % separated boxes
  for j=1:length(Ag)-1
    lab=[num2str(Ag(j)) '-' num2str(Ag(j+1)) ' km h^{-1}'];
    if j==1 & hasL & IncHiLow
      lab=['<',num2str(Ag(2)) ' km h^{-1}'];
    end
    if j==length(Ag)-1 & hasH & IncHiLow
      lab=['>=',num2str(Ag(j)) ' km h^{-1}'];
    end
    y1=y0+h;
