...
zeroStageMetPath = fullfile(biomet_path('yyyy','TPAG','Met'));
zeroStageFluxPath = fullfile(biomet_path('yyyy','TPAG','Flux'));

firstStageMetPath = fullfile(biomet_path('yyyy','TPAG','Met/Clean'));
firstStageFluxPath = fullfile(biomet_path('yyyy','TPAG','Flux/Clean'));

secondStagePath = fullfile(biomet_path('yyyy','TPAG','Clean/SecondStage'));
thirdStagePath = fullfile(biomet_path('yyyy','TPAG','Clean/ThirdStage'));
thirdStageUStarPath = fullfile(biomet_path('yyyy','TPAG','Clean/ThirdStage_Default_Ustar'));
...
makeplot = 1;
saveplot = 1;
% savepath = ['/Users/rosiehoward/Documents/UBC/Micromet/Matlab/local_personal_plots/TurkeyPoint_Altaf/' siteID '/' num2str(year) '/Clean/ThirdStage/'];
% savepath = ['/Users/rosiehoward/Documents/UBC/Micromet/Matlab/local_personal_plots/TurkeyPoint_Altaf/' siteID '/' num2str(year) '/Clean/ThirdStage_Default_Ustar/'];
...
if makeplot == 1
    ...
    % load time vector
    tv = read_bor(fullfile(firstStageMetPath,'clean_tv'),8,[],year);
    % convert time vector to Matlab's datetime
    tv_dt = datetime(tv,'ConvertFrom','datenum');

    % load variables
    list_files = dir([biomet_database_default '/' num2str(year) '/' siteID '/Clean/ThirdStage']);
    % list_files = dir([biomet_database_default '/' num2str(year) '/' siteID '/Clean/ThirdStage_Default_Ustar']);
        ...
        for i = 1:length(list_files)
            ...
            % load data
            var = read_bor(fullfile(thirdStagePath,value),[],[],year);
            % var = read_bor(fullfile(thirdStageUStarPath,value),[],[],year);
